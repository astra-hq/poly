...
Poly is a self-contained desktop application built with [Tauri](https://tauri.app/). It combines a Rust-based backend with a Next.js frontend into a single, efficient, and cross-platform application.
...
- **Path**: `~/.poly/poly.yml`
- **Legacy path** (auto-migrated on first launch): `~/.resourcefully/resourcefully.yml`
...
- **Legacy fallback**: If no secret is found in the Poly store, the app checks legacy `com.meetily.secrets` (macOS Keychain) and `~/.resourcefully/secrets.yml` and copies any found values forward. Legacy secrets are never deleted.
...
> **Legacy migration note:** On first launch after the Poly rebrand, the app automatically copies config from `~/.resourcefully/resourcefully.yml` and secrets from `com.meetily.secrets` / `~/.resourcefully/secrets.yml` into the new Poly paths. No data is deleted from the legacy locations.
